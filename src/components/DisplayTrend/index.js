...
class DisplayTrend extends Component {
  ...
  render() {
        ...
        if ( (this.props.store.app.trendStatus) && (this.props.store.app.getChartData) ) {
            ...
            //plotLines.push({ label: {text:'First Freeze (Median)', y:180}, color: 'rgba(135,206,250,1.0)', width: 6, zIndex: 4, value: moment(data.datesOfFirstFreeze_15yr[7], 'YYYY-MM-DD') })
            //plotLines.push({ label: {text:'Last Freeze (Median)', y:180}, color: 'rgba(135,206,250,1.0)', width: 6, zIndex: 4, value: moment(data.datesOfLastFreeze_15yr[7], 'YYYY-MM-DD') })
            //plotBands.push({ label: {text:''}, color: 'rgba(0,0,255,0.1)', from: moment(data.datesOfFirstFreeze_15yr[0], 'YYYY-MM-DD'), to: moment(data.datesOfFirstFreeze_15yr[data.datesOfFirstFreeze_15yr.length-1], 'YYYY-MM-DD')})
            //plotBands.push({ label: {text:''}, color: 'rgba(0,0,255,0.1)', from: moment(data.datesOfLastFreeze_15yr[0], 'YYYY-MM-DD'), to: moment(data.datesOfLastFreeze_15yr[data.datesOfLastFreeze_15yr.length-1], 'YYYY-MM-DD')})
            ...
            var chartConfig = {
                 ...
                 xAxis: { type: 'datetime', startOnTick: false, endOnTick: false,
                     //min: getXaxisMin(),
                     max: getXaxisMax(), labels: { align: 'center', x: 0, y: 20 },
                     plotLines: plotLines,
                     plotBands: plotBands,
                 ...
                 yAxis: [{ title:{ text:'Cumulative GDD', style:{"font-size":"14px", color:"#000000"}}, min: 0, gridZIndex:1, labels:{style:{color:"#000000"}}},
                     //{max: 8, labels: {enabled: false}, title: {enabled: false} },
                     { title:{ text:'First/Last Freeze (# yrs)', align:'low', style:{"font-size":"14px", color:"#0000FF"}}, min: 0, max: 8, labels:{style:{color:"#0000FF"}}, opposite: true },
                 ],
                 series: [{
